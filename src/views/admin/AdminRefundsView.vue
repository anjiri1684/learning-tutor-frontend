...
<template>
  <div class="min-h-screen bg-black text-white p-6 relative isolate overflow-hidden font-sans selection:bg-purple-500 selection:text-white">
    ...
    <div class="max-w-6xl mx-auto">
      <h1 class="text-3xl font-bold tracking-tight text-white drop-shadow-[0_0_10px_rgba(168,85,247,0.5)] mb-8">Pending Refund Requests</h1>
      ...
      <div class="bg-gray-900/60 backdrop-blur-md rounded-2xl shadow-xl shadow-purple-900/10 border border-white/10 overflow-hidden">
          ...
          <table class="w-full text-left whitespace-nowrap">
            <thead>
              <tr class="bg-white/5 text-gray-300 text-sm uppercase tracking-wider border-b border-gray-700">
                ...
                <th class="p-4 font-semibold w-1/3">Reason</th>
                ...
                <td class="p-4">
                   <div class="flex items-center gap-3">
                      <div class="h-8 w-8 rounded-full bg-gray-800 flex items-center justify-center text-xs font-bold text-gray-400 border border-gray-700">
                         {{ req.Booking.Student.full_name.charAt(0).toUpperCase() }}
                      </div>
                      <span class="font-medium text-white">{{ req.Booking.Student.full_name }}</span>
                   </div>
                </td>
                <td class="p-4 font-bold text-red-400">${{ req.Amount.toFixed(2) }}</td>
                <td class="p-4">
                   <p class="text-sm text-gray-400 truncate max-w-xs" :title="req.RefundReason">
                      {{ req.RefundReason }}
                   </p>
                </td>
                <td class="p-4 text-gray-400 text-sm font-mono">{{ formatDate(req.CreatedAt) }}</td>
        ...
        <div v-else class="p-12 text-center text-gray-500 flex flex-col items-center justify-center">
           <svg class="w-12 h-12 mb-4 text-gray-700" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"></path></svg>
           <p>There are no pending refund requests.</p>
        </div>
      </div>
    </div>
    ...
    <Teleport to="body">
      <div v-if="showModal" class="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-[100] p-4 font-sans text-white">
        <div class="bg-gray-900 border border-white/10 rounded-2xl shadow-2xl shadow-purple-900/40 w-full max-w-lg overflow-hidden transform transition-all scale-100">
          <div class="p-8">
            <h3 class="text-xl font-bold text-white mb-2">Process Refund</h3>
            <p class="text-gray-400 text-sm mb-6">
               Processing refund for <span class="text-white font-semibold">{{ selectedRequest.Booking.Student.full_name }}</span>
            </p>

            <div class="bg-black/30 p-4 rounded-xl border border-white/5 mb-6 space-y-3">
               <div class="flex justify-between items-center border-b border-gray-800 pb-2">
                  <span class="text-gray-400 text-sm">Requested Amount</span>
                  <span class="text-xl font-bold text-red-400">${{ selectedRequest.Amount.toFixed(2) }}</span>
               </div>
               <div>
                  <span class="text-gray-500 text-xs uppercase tracking-wide">Reason</span>
                  <p class="text-gray-300 italic text-sm mt-1">"{{ selectedRequest.RefundReason }}"</p>
               </div>
            </div>
