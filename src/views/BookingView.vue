...
const handlePaystackPayment = async () => {
  ...
  message.value = { type: 'info', text: 'Connecting to Paystack...' };
  isSubmitting.value = true;
  ...
  try {
    ...
    const handler = PaystackPop.setup({
      ...
      amount: Math.round(priceInKES.value * 100),
      currency: 'KES',
      ref: paymentId,
    ...
    message.value = { type: 'error', text: error.message || 'Could not initiate Paystack transaction.' };
    isSubmitting.value = false;
  }
};
          ...
          >
            <div class="flex justify-between items-center">
               <h3 class="font-bold text-blue-400 flex items-center gap-2">
                  <span class="w-2 h-2 rounded-full bg-blue-500"></span> Paystack
               </h3>
               <span class="text-xs font-mono text-gray-400">KES {{ priceInKES.toFixed(2) }}</span>
            </div>
